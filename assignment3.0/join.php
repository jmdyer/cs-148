...
<html lang="en">
    ...
    <body>
        ...
        <p>q01 <a href="../assignment3.0/q01.php">SQL:</a> SELECT distinct fldCourseName FROM tblCourses, tblEnrolls WHERE tblCourses.pmkCourseId = tblEnrolls.fnkCourseId AND tblEnrolls.fldGrade = '100' ORDER BY fldCourseName ASC</p>
        <p>q02 <a href="../assignment3.0/q02.php">SQL:</a> SELECT distinct fldDays, fldStart, fldStop FROM tblSections, tblTeachers WHERE tblTeachers.pmkNetId = tblSections.fnkTeacherNetId AND tblTeachers.fldLastName = "Snapp" ORDER BY fldStart ASC</p>
        <p>q03 <a href="../assignment3.0/q03.php">SQL:</a> SELECT distinct tblCourses.fldCourseName, fldDays, fldStart, fldStop FROM tblSections, tblCourses, tblTeachers WHERE tblCourses.pmkCourseId = tblSections.fnkCourseId AND tblTeachers.pmkNetId = tblSections.fnkTeacherNetId AND tblTeachers.fldLastName = "Horton" ORDER BY fldStart ASC</p>
        <p>q04 <a href="../assignment3.0/q04.php">SQL:</a> SELECT fldCRN, tblStudents.fldFirstName, tblStudents.fldLastName FROM tblSections, tblEnrolls, tblStudents
...
ORDER BY tblEnrolls.fldGrade DESC</p>
        <p>q08 <a href="../assignment3.0/q08.php">SQL:</a> SELECT tblTeachers.fldFirstName, tblTeachers.fldLastName, COUNT(fnkStudentId) AS "total", fldSalary, fldSalary/COUNT(fnkStudentId) AS "IBB Value" FROM tblTeachers, tblSections, tblEnrolls
WHERE tblTeachers.pmkNetId = tblSections.fnkTeacherNetId
AND tblSections.fldCRN = tblEnrolls.fnkSectionId
AND fldType <> "LAB"
GROUP BY tblTeachers.fldLastName, tblTeachers.fldFirstName
ORDER BY `IBB Value` ASC</p>
